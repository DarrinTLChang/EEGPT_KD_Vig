...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def __init__(self, load_path="EEGvigilance/stage1_labram_fewshot/best.ckpt"):
        super().__init__()    
        ...
        pretrain_labram_ckpt = torch.load(load_path)
        state_dict_labram = pretrain_labram_ckpt['state_dict']
        eeg_encoder_state = OrderedDict({k.replace("eeg_encoder.", ""): v for k, v in state_dict_labram.items() if k.startswith("eeg_encoder.")})
        eeg_mapper_state = OrderedDict({k.replace("eeg_mapper.", ""): v for k, v in state_dict_labram.items() if k.startswith("eeg_mapper.")})
        eeg_classifier_state = OrderedDict({k.replace("eeg_classfier.", ""): v for k, v in state_dict_labram.items() if k.startswith("eeg_classfier.")})
