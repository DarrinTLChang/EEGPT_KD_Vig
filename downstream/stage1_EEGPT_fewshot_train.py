...
class LitEEGPTCausal(pl.LightningModule):
    def __init__(self, load_path="/Users/darrin/Documents/best.ckpt"):
        ...
        self.num_class = 2
        self.future_steps = 5 * 5

        # --- encoder ---
        self.eeg_encoder = EEGTransformer(
        ...
        self.chans_id = self.eeg_encoder.prepare_chan_ids(use_channels_names)
        ...
        if load_path is not None:
            ...
            print(f"[CKPT] loaded from {load_path}")
            print(f"[CKPT] missing keys: {len(missing)}  unexpected keys: {len(unexpected)}")

        # IMPORTANT: do NOT call _reset_parameters() after loading!
        # If you *must* initialize when no ckpt is provided, do it conditionally:
        # if load_path is None: self._reset_parameters()
...
if __name__=="__main__":
    ...
    batch_size=32
    train_dataset, test_dataset = build_dataset_fewshot("my_binary_5xtr_2625")
