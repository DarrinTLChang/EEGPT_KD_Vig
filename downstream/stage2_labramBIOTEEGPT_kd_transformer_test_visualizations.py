...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def __init__(self, load_path="EEGvigilance/stage2_labramBIOTEEGPT_kd_fewshot_transformer/best.ckpt"):
        super().__init__()    
        ...
        eeg_encoder_state_biot = OrderedDict({k.replace("eeg_encoder_biot.", ""): v for k, v in state_dict.items() if k.startswith("eeg_encoder_biot.")})
        eeg_mapper_state_biot = OrderedDict({k.replace("eeg_mapper_biot.", ""): v for k, v in state_dict.items() if k.startswith("eeg_mapper_biot.")})
        eeg_classifier_state_biot= OrderedDict({k.replace("eeg_classfier_biot.", ""): v for k, v in state_dict.items() if k.startswith("eeg_classfier_biot.")})
        chan_conv_state_biot = OrderedDict({k.replace("chan_conv_biot.", ""): v for k, v in state_dict.items() if k.startswith("chan_conv_biot.")})
        ...
        eeg_encoder_state_labram = OrderedDict({k.replace("eeg_encoder_labram.", ""): v for k, v in state_dict.items() if k.startswith("eeg_encoder_labram.")})
        eeg_mapper_state_labram = OrderedDict({k.replace("eeg_mapper_labram.", ""): v for k, v in state_dict.items() if k.startswith("eeg_mapper_labram.")})
        eeg_classifier_state_labram = OrderedDict({k.replace("eeg_classfier_labram.", ""): v for k, v in state_dict.items() if k.startswith("eeg_classfier_labram.")})
        ...
        fmri_classifier_state = OrderedDict({k.replace("fmri_classifier.", ""): v for k, v in state_dict.items() if k.startswith("fmri_classifier.")})
        self.fmri_encoder.load_state_dict(fmri_encoder_state)
        self.fmri_mapper.load_state_dict(fmri_mapper_state)
        self.fmri_classifier.load_state_dict(fmri_classifier_state)
    ...
    def forward(self, eeg, fmri, is_train=True):
        if is_train == True:
            ...
            fmri_all_feats = fmri_feats.reshape(fmri_feats.shape[0], -1)
            fmri_logits = self.fmri_classifier(fmri_all_feats)
            return fmri_all_feats, fmri_logits, fmri_all_feats, fmri_logits, fmri_all_feats, fmri_logits, fmri_all_feats, fmri_logits
    ...
    def test_step(self, batch, batch_idx):
        fmri, eeg, physio, eeg_index_linear_raw, eeg_index_linear_smoothed, eeg_index_binary, alpha_theta_ratio, vigilance_seg = batch
        eeg_all_feats, eeg_logits, eeg_all_feats_labram, eeg_logits_labram, eeg_all_feats_biot, eeg_logits_biot, fmri_all_feats, fmri_logits = self.forward(eeg, fmri, is_train=False) 
        final_loss, losses = self.criterion.single_output_forward(eeg_all_feats, eeg_logits, eeg_all_feats_labram, eeg_logits_labram, eeg_all_feats_biot, eeg_logits_biot, fmri_all_feats, fmri_logits, vigilance_seg)
        self.running_scores["test"].append((vigilance_seg.detach().cpu(), fmri_logits.detach().cpu()))
        return final_loss
    ...
    def harvest_feats(self, fmri):
        ...
        fmri_all_feats = fmri_feats.reshape(fmri_feats.shape[0], -1)
        fmri_logits = self.fmri_classifier(fmri_all_feats)
        print(f"fmri_all_feats: {fmri_all_feats.shape}")
        print(f"fmri_logits: {fmri_logits.shape}")
        return fmri_all_feats, fmri_logits


def save_feats(model, dataloader, dataset_name):
    model.eval()
    model.cuda()  
    ...
    with torch.no_grad():
        for batch in tqdm.tqdm(dataloader):
            if dataset_name != "eegfmri_vu_pat_alphatheta_smallinterval_1024":
                fmri, eeg, physio, eeg_index_linear_raw, eeg_index_linear_smoothed, eeg_index_binary, alpha_theta_ratio, vigilance_seg = batch
                device = model.device
                feats, logits = model.harvest_feats(fmri.to(device))
                all_feats.append(feats.cpu())
                all_logits.append(logits.cpu())
                all_gts.append(vigilance_seg.cpu())
            else:
                fmri = batch
                device = model.device
                feats, logits = model.harvest_feats(fmri.to(device))
    ...
    print(f"all_feats_tensor: {all_feats_tensor.shape}")
    print(f"all_logits_tensor: {all_logits_tensor.shape}")
    if dataset_name != "eegfmri_vu_pat_alphatheta_smallinterval_1024":
        all_gts_tensor = torch.cat(all_gts, dim=0)
        print(f"all_gts_tensor: {all_gts_tensor.shape}")
    
    save_dir = "EEGvigilance/stage2_labramBIOTEEGPT_kd_fewshot_transformer/" + dataset_name + "/"
    os.makedirs(save_dir, exist_ok=True)
    torch.save(all_feats_tensor, os.path.join(save_dir, "fmri_feats.pt"))
    torch.save(all_logits_tensor, os.path.join(save_dir, "fmri_logits.pt"))
    if dataset_name != "eegfmri_vu_pat_alphatheta_smallinterval_1024":
        torch.save(all_gts_tensor, os.path.join(save_dir, "gts.pt"))
...
if __name__=="__main__":
    ...
    save_feats(model, eegfmri_vu_train_loader, "eegfmri_vu_alphatheta_smallinterval_1024_train")
    save_feats(model, eegfmri_vu_test_loader, "eegfmri_vu_alphatheta_smallinterval_1024_test")
    save_feats(model, nihecr_zeroshot_loader, "NIHECR_alphatheta_smallinterval_1024_zeroshot")
    save_feats(model, eegfmri_vu_pat_zeroshot_loader, "eegfmri_vu_pat_alphatheta_smallinterval_1024")
